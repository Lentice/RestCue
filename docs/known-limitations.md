...
- 以下功能尚未實作：提醒逾時自動淡出、全螢幕降級、休息引導 (Break Guide) 語音／音效、session lock 與 power events、暫停與專注模式、停用狀態、tray 狀態圖示切換、完整選單命令。延後 (Snooze)、忽略 (Ignore) 與 AutoDismissed 已實作於 WorkCycleTracker 與 MainWindow，但 ReminderDismissedEventArgs typed events 尚未持久化（#10）。
