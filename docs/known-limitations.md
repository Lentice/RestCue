...
- WorkCycleTracker 累積工作時間的最小粒度為 5 秒（對應輪詢間隔），從 Idle 恢復的第一個 Tick 不累積，最多損失 5 秒。
...
- 資料庫以 WAL 模式寫入（`PRAGMA journal_mode=WAL`，migration 時設定一次），且使用記錄寫入連線採 `PRAGMA synchronous=NORMAL`：`-wal`／`-shm` 檔案會常駐於資料庫目錄，且作業系統或電源突然中斷時，最近尚未 checkpoint 的少量使用事件可能遺失（資料庫本身不會損毀）。設定與應用程式規則以單筆 commit 寫入，不受此影響。
- 使用事件為批次寫入：`BackgroundUsageEventWriter` 會把佇列中已排隊的事件（每批最多 64 筆）合併成一個 transaction，因此事件從產生到落地存在一個極短的延遲窗口，此期間行程被強制結束會遺失該批事件。
