...
- 儲存設定後，可即時套用者（前景程式名稱蒐集、減少動態效果、休息引導模式、提醒透明度）立即生效；提醒引擎的建構參數（工作間隔、各停頓／離席門檻、最長提醒等待、休息長度、延後長度、提醒顯示時間、提醒重試冷卻、休息債務門檻、專注模式長度）須待下次啟動，因為重建引擎會清掉累積工作時間，等同使用者未要求的可信重設。儲存確認訊息會逐項列出真正需要重新啟動的設定。「延後長度」列為需重新啟動：延後期限由引擎持有，若只即時更新提醒視窗的標示，標示會承諾引擎不會遵守的時間。
...
- 執行中休息期間無法「暫停提醒」：引擎不接受從休息中轉入 Paused，而命令可用性刻意與引擎接受的轉換完全一致。要在休息中暫停，需先取消休息。「停用提醒」在休息中可用，且會明確取消並記錄該次休息。
